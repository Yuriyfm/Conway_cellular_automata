...
let CellsGrid = () => {
    ...
    let cellsGrid = createCellsGrid()
    let [cells, setCells] = useState(cellsGrid)
    ...
    let isSurvive = (y, x) => {
        ...
        } else { rows = cells.slice(y - 1, y + 2)}

        if (x === 0) {
            block = rows.map(row => row.slice(x, x + 2))
        } else if (y === cells.length -1) {
            block = rows.map(row => row.slice(x - 1, x + 1))
        } else { block = rows.map(row => row.slice(x - 1, x + 2))}
    ...
    return (
        <div>
            <div style={{margin: 10}}>
                <button style={{marginRight: 20}} onClick={resetGrid}>обновить</button>
                <button onClick={() => {
                    startLifeProcess()
                }}>старт</button>
            </div>
